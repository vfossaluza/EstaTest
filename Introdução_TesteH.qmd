---
title: "Introdução à Teste de Hipótese"
...
Inicialmente calculamos a função de verossimilhança:

$$
V(x) =  \prod^{n}_{i=1} f(x_i | \theta) = \prod^{n}_{i=1} \theta \, e^{\theta -x_i} =
...
Com isso, a razão de verossimilhança generalizada $\lambda(x)$ é

$$
\lambda(x) = \frac{\theta_0^n e^{\theta_0\sum_{i=1}^n x_i}}{\hat{\theta^n} e^{-\hat{\theta} \sum_{i=1}^n x_i}} \text{ , em que } \theta_0 \text{ satisfaz } H_0
$$
Para a amostra 1:

$$
\hat{\theta} = \frac{2}{1} = 2 \Rightarrow \lambda(x) = \frac{1^2 e^{-1}}{2^2 e^{-2}} = \frac{e}{4} \approx 0,679 
$$

Para a amostra 2:

$$
\hat{\theta} = \frac{2}{3} \Rightarrow \lambda(x) = \frac{(\frac{2}{3})^2 e^{2/3}}{(\frac{2}{3})^2 e^{2/3}} = 1
$$
