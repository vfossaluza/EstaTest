...
$$
\theta | \mathbf{X} = \mathbf{x} \sim \sum_{i=1}^{10} X_i = x \sim Beta(a+x, b+n-x)
$$
...
($iii$) $P( \theta \in \Theta_0 | \mathbf{x})$
...
$$
Usamos $\theta = 1/2$ porque é o valor de maximiza $H_0$.
...
e_value <- function(a, b, theta0 = 1/2) {
  f <- function(theta) dbeta(theta, a, b)
  f0 <- f(theta0) # valor da densidade no ponto da hipótese
  
  # função g(t) = f(t) - f(theta0)
  g <- function(theta) f(theta) - f0
  
  # achar onde a densidade cruza f(theta0)
  limite_inf <- (a - 1) / (a + b - 2)
  root <- uniroot(g, interval = c(limite_inf, 1))$root
  
  # agora precisamos integrar onde f(t) >= f(theta0)
  mode <- (a - 1) / (a + b - 2)
  
  # integral da densidade na região de suporte da hipótese
  ev <- integrate(f, lower = mode, upper = root)$value
  
  return(ev)}

e_value <- function(a, b, theta0 = 0.5) {

  f <- function(t) dbeta(t, a, b)
  f0 <- f(theta0)  # densidade no ponto da hipótese
  
  mode <- (a - 1) / (a + b - 2) # moda da beta
  if (a <= 1 || b <= 1) mode <- theta0  # casos degenerados
  
  # função g(t) = f(t) - f0
  g <- function(t) f(t) - f0
  
  # tentar achar raiz à direita do modo
  root <- try(uniroot(g, c(theta0, 1), extendInt = "yes")$root,
              silent = TRUE)
  if (inherits(root, "try-error")) return(1)  # se não achar, e-value = 1
  
  # integrar onde f(t) >= f0
  ev <- integrate(f, lower = mode, upper = root)$value
  return(ev)}
```
...
# Função para calcular e-value
e_value_beta <- function(a, b, theta0 = 0.5) {
  ...
  root <- try(uniroot(g, c(theta0, 1), extendInt = "yes")$root,
              silent = TRUE)
  if (inherits(root, "try-error")) return(1)
  ev <- integrate(f, lower = mode, upper = root)$value
  return(ev)}
...
for(i in 1:length(x_vals)) {
  ...
  # e-value
  tabela$e_value[i] <- e_value_beta(alp, bet, theta0 = 0.5)}

tabela_final <- round(tabela, 4)
print(tabela_final)
```
